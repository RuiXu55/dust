...
std::vector <double> Z,N_h,Ts,Vz,Sc,g1,tman1,
...
for(int i=0;i<Num-1;i++){
  g1.push_back((N_h[i+1]*Al(Z[i+1])*Cs(Z[i+1])+
          N_h[i]*Al(Z[i])*Cs(Z[i]))/2.0);
}
...
for(int j=0;j<NDsize;j++){
  ...
  for(int i=0;i<Num-1;i++){
   ds.g1.push_back((N_h[i+1]*Al(Z[i+1])*Cs(Z[i+1])/ds.Sc[i+1]+
      N_h[i]*Al(Z[i])*Cs(Z[i])/ds.Sc[i])/2.0);
  }
...
while (time<Evo_time){
  ...
  if (outtime>1000.0){
    ...
    iter += 1.0;
    f<<"timestep ="<<iter<<"  "<<Num<< "  "<<NDsize+2<<endl;
    for(int i=0;i<Num;i++){
  ...
  for (int i=0;i<NDsize;i++){
    ...
    maxman = msiteden*ngr;
    nlayer[i] = min(1000.0,Grain[i].man[j]/maxman);

    tman1[i]= Grain[i].man[j]+dt*(n_neu[j]*Grain[i].ad[j]-
      Grain[i].man[j]*Grain[i].des[j]/max(1.0,nlayer[i]));
    //tman = Grain[i].man[j]+n_neu[j]*Grain[i].ad[j]-
    // Grain[i].man[j]*Grain[i].des[j];
    if(tman1[i]<0.0)
      tman1[i] = 0.0;
    if(tman1[i]>n_neu[j]+Grain[i].man[j])
      tman1[i] = n_neu[j] + Grain[i].man[j];
    //n_neu[j] += Grain[i].man[j]-tman[i];
    //Grain[i].man[j] = tman;
    }
    for(int i=0;i<NDsize;i++){
    n_neu[j] += Grain[i].man[j]-tman1[i];
    Grain[i].man[j] = tman1[i];}
  }
  for (int i=0;i<NDsize;i++){
    // boundary condition 
